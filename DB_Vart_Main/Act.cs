...
namespace DB_Vart_Main
{
    public partial class Act : Form
    {
        ...
        public void Calc(SqlConnection connection, String contract)
        {
            ...
            //new!!!
            startYear = new DateTime(2019, 2, 1);
            for (int y = 0; y < list.Count - 1; y++)
            {
                if (list.Count == 1)
                    break;
                if (list[y].Date < startYear && list[y + 1].Date > startYear)
                {
                    k = y;
                    break;
                }
            ...
            int deb = reader.GetInt32(0);
            reader.Close();
            ListViewItem it = new ListViewItem(new string[] { "Долг на 01.02.2019", deb.ToString() });//, "Платежи:", pa.ToString() });
            listViewAct.Items.Add(it);
            sumFee = deb;

            if (endYear < startYear)
            {
                it = new ListViewItem(new string[] { "Договор прекращен",  "раньше чем 01.02.2019" });
                listViewAct.Items.Add(it);
            }
        ...
        public void Calc(SqlConnection connection, String contract, ref ListView view)
        {
            SqlCommand command = new SqlCommand("SELECT Monthly_fee FROM Main WHERE Contract_num = '" + contract + "'", connection);
            SqlDataReader reader = command.ExecuteReader();
            ...
            view.Items[0].SubItems[5].Text = (sumFee - sumPay).ToString();
        }
